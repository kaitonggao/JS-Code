// one class to create the objet person, ablum, movie, ****Model
//take arguments : class x { constructor (age,name){this. etc}{}



//***View */ class for the UI which contains methods
//this UI has 
//creates the html for the UI Display -> insert things like age, album, artist those variables.

//clear field method 

//remove something/object 

//controller combines ui and model
//w/ user values -> 
// instantiate -> make a new copy of a class with user inputed values

//call relevant methods

//seperate event handler for removing/deleting 

//event handler with function 